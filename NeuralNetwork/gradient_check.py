...
def check_gradient(f, x, delta=1e-5, tol = 1e-4):
	...
	while not it.finished:
		...
		x_plus_delta = orig_x.copy()
		x_plus_delta[ix] += delta
  
		x_minus_delta = orig_x.copy()
		x_minus_delta[ix] -= delta
  
		numeric_grad_at_ix = (f(x_plus_delta)[0] - f(x_minus_delta)[0]) / (2.0 * delta)

		np.testing.assert_allclose(numeric_grad_at_ix, analytic_grad_at_ix, rtol=tol, atol=tol, \
      	err_msg=f"Gradients are different at {ix}. Analytic: {analytic_grad_at_ix}, Numeric: {numeric_grad_at_ix}")
