...
class KNN:
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.zeros((num_test, num_train), np.float32)
        dists = np.sum(np.abs(np.expand_dims(X, axis=1) - np.expand_dims(self.train_X, axis=0)), axis=2)
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            test_i = np.argsort(dists[i])[:self.k]
            k_labels = [self.train_y[j] for j in test_i]
            most = Counter(k_labels).most_common(1)
            pred[i] = most[0][0]
